...
export const metadata: Metadata = { title: 'Páginas de vendas | Trakeamento' };

/**
 * Sites rastreados de um cliente: a tag para colar na página e as URLs
 * de webhook para cadastrar na plataforma de checkout.
 *
 * Só administrador. As URLs de webhook carregam o token secreto do site;
 * a tela do cliente mostra os números, nunca estas URLs.
 */
export default async function PaginaSitesAdmin({ params }: { params: Promise<{ cliente: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      <PageHero
        titulo={`Páginas de vendas — ${conta.account_name}`}
        descricao="Rastreio de página, formulário, checkout e compra. Uma tag por site; um webhook por plataforma de checkout."
        acoes={
          <Link href="/admin/clientes" className="btn-ghost px-2 py-1 text-xs">
            Voltar aos clientes
          </Link>
        }
      />
      ...
      {sites.map((site) => {
        ...
        return (
          <div key={site.id} className="card space-y-4 p-5">
            ...
            <details className="rounded-[var(--radius-control)] border border-[var(--border-subtle,#e4e4e7)] p-3">
              ...
              <div className="pt-3">
                <FormularioSite
                  ...
                  site={{
                    ...
                    kommo_pipeline_id: site.kommo_pipeline_id,
                    kommo_status_id: site.kommo_status_id,
                    envia_kommo: site.envia_kommo,
                    ativo: site.ativo,
                  }}
                />
              </div>
      ...
      <div className="card space-y-2 p-5 text-xs text-[var(--text-secondary)]">
        <h2 className="text-sm font-medium text-[var(--text-primary,inherit)]">Como a página conversa com o script</h2>
        <p>

...
describe('fmtHoraRelativa', () => {
  ...
  test('mais de uma hora no mesmo dia vira a hora do relógio', () => {
    assert.equal(fmtHoraRelativa('2026-05-10 09:05:00', AGORA), '09:05');
  });
  ...
  test('da hora em diante o corte é o dia em São Paulo, não "24h atrás"', () => {
    ...
    assert.equal(fmtHoraRelativa('2026-05-10 23:50:00', manha), 'ontem');
    ...
    assert.equal(fmtHoraRelativa('2026-05-10 23:50:00', meiaNoite), '40 min');
  });

  test('a semana em dias, e depois a data', () => {
    assert.equal(fmtHoraRelativa('2026-05-08 10:00:00', AGORA), '2 d');
    assert.equal(fmtHoraRelativa('2026-05-01 10:00:00', AGORA), '01/05');
  });
  ...
  test('sem data não inventa texto, e futuro não vira número negativo', () => {
    ...
    assert.equal(fmtHoraRelativa('2026-05-10 15:00:00', AGORA), '15:00');
  });
});
